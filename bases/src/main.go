...
func arrays() {
	...
	value, ok := m["Jose"]
	//si la llave no existe no devuelve nada
	value2 := m["Joseh"]
	fmt.Println(value2, "-")
	//si se desea saber si el valor existe se guarda en una segunda variable
	value, ok = m["Josep"]
...
func errormanage(val string) {
	valuee, err := strconv.ParseInt(val, 0, 64)
	fmt.Println("Valor: ", valuee, " | Error: ", err)
	if err != nil {
